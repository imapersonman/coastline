...
export class UserExpectation<D> { constructor(readonly expectation: string, readonly data: D) {} }
export const is_user_expectation = (u: unknown): u is UserExpectation<unknown> => u instanceof UserExpectation

export class ConstructorEvent<D> { constructor(readonly event: string, readonly data: D) {} }
...
export const default_expect: Expect<UserExpectation<unknown>> = {
    ...
    user_to_give_tactic_or_sub_problem_id: () =>
        new UserExpectation('user_to_give_tactic_or_sub_problem_id', undefined),
...
export const default_emit: EmitEvent<ConstructorEvent<unknown>> = {
    ...
    user_gave_main_problem: (mp: Sequent) =>
        new ConstructorEvent('user_gave_main_problem', { main_problem: mp }),
    ...
    user_gave_sub_problem: (sub_problem: SubProblem) =>
        new ConstructorEvent('user_gave_sub_problem', sub_problem),
    ...
    finished_tactic: (valid_proof_insert: ValidProofInsert) =>
        new ConstructorEvent('finished_tactic', valid_proof_insert),
...
export function* get_valid_main_problem_from_user(sig: Sig, expect: Expect<any>, emit_event: EmitEvent<any>) {
    let main_problem = yield expect.user_to_give_main_problem()
    ...
    while (is_sequent_error(main_problem_check)) {
        ...
        main_problem = yield expect.user_to_give_main_problem()
...
export function* get_user_selected_tactic_and_sub_problem(tactics: Record<string, Tactic<any>>, sub_problems: Record<number, SubProblem>, emit_event: EmitEvent<any>, expect: Expect<any>): Generator<any, [string, Tactic<any>, number, SubProblem], any> {
    ...
    while (!(tactic_id !== undefined && tactic !== undefined && sub_problem_id !== undefined && selected_sub_problem !== undefined)) {
        const tactic_or_sub_problem_id = yield expect.user_to_give_tactic_or_sub_problem_id()
        if (tactic_or_sub_problem_id.type === "TacticId") {
            ...
            selected_sub_problem = yield* emit_defined_or_undefined<number, SubProblem>(sub_problems, tactic_or_sub_problem_id.value, emit_event.user_gave_sub_problem, emit_event.user_gave_bad_sub_problem)
        }
    }
...
export function* notify_user_about_insert(
    ...
    const checked_proof_insert = check_proof_insert(sig, seq, insert.new_conclusions, insert.fragment, m, v)
    if (!is_valid_proof_insert(checked_proof_insert))
        throw new Error(exceptions.invalid_proof_insert(checked_proof_insert))
    yield emit_event.finished_tactic(checked_proof_insert)
...
export function* run_interaction<Exp, Emi>(spec, expect: Expect<Exp>, emit_event: EmitEvent<Emi>, exceptions: Exceptions): Generator<Exp | Emi, void, unknown> {
    ...
    while (!isEmpty(sub_problems)) {
        ...
        while (is_request(tactic_state_it.value)) {
            ...
            if (is_user_error(transformed_parameter))
                yield* notify_user_about_error(spec.user_errors, transformed_parameter, emit_event, exceptions)
            else {
                const [response, transformed_response] = yield* run_response_interaction(request, transformed_parameter, request_definition.response, spec.user_errors, emit_event, expect, exceptions)
        ...
        if (is_user_error(tactic_state_it.value))
            ...
            const checked_proof_insert = yield* notify_user_about_insert(spec.sig, current_sub_problem.sequent, tactic_state_it.value, m, v, emit_event, exceptions)
...
class ProofConstructor extends EventEmitter {
    constructor(spec: VerifiedInteractionSpecification) {
        super()
    }
