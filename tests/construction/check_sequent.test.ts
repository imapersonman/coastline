...
import { clist, con, type_k } from "../../src/lambda_pi/shorthands"
...
describe("check_sequent makes sure a sequent is valid under a given signature", () => {
    it("succeeds on an empty ctx and valid conclusion", () => 
        expect(
            check_sequent(mk_sig([a, type_k]), sequent(mk_map(), con("a")))
        ).toEqual(
            []
        )
    )
    it("succeeds on a non-empty ctx and valid conclusion", () =>
        expect(
            check_sequent(mk_sig([a, type_k], [b, type_k]), sequent(mk_map(["x", con("a")]), con("b")))
        ).toEqual(
            []
        )
    )
    it("fails on an invalid non-empty ctx and valid conclusion", () =>
        expect(
            check_sequent(mk_sig(), sequent(mk_map(["x", con("a")]), type_k))
        ).toEqual(
    ...
    it("fails on a valid non-empty ctx and invalid conclusion", () => 
        expect(
            check_sequent(mk_sig([a, type_k], [b, type_k]), sequent(mk_map(["x", con("a")], ["y", con("b")]), type_k))
        ).toEqual(
    ...
    it("fails on an invalid non-empty ctx and invalid conclusion, but reports the ctx error", () =>
        expect(
            check_sequent(mk_sig([b, type_k]), sequent(mk_map(["x", con("a")], ["y", con("b")]), type_k))
        ).toEqual(
            new BadEntry("x", new UndeclaredConstant(con("a")))
        )
    )
