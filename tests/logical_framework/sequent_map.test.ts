...
describe('sequent map', () => {
    ...
    describe('contains', () => {
        test('empty does not contain 1', () => expect(empty.contains(empty_tk_seq)).toEqual(false))
        test('empty does not contain 2', () => expect(empty.contains(Bc_Aa_c_seq)).toEqual(false))
        test('non-empty does not contain', () => expect(two_with_ctx_names_not_types_permuted_and_same_conclusion.contains(Ba_Ab_c_seq)).toEqual(true))
    })
    ...
    describe('get', () => {
        ...
        test('non-existing value from non-empty with non-empty ctx', () => expect(single_with_non_empty_ctx.get(sequent(mk_map(['A', a], ['C', b]), c))).toEqual(undefined))
        ...
        test('permuted ctxs but different conclusions should be treated as different', () => expect(two_with_ctx_names_not_types_permuted_and_same_conclusion.get(Ba_Ab_c_seq)).toEqual(4))
    })
